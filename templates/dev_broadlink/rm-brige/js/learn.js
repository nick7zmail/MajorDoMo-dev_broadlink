        ...
        function init() { 
            var app = angular.module("app", [])
                .controller("RMController", function ($scope, $http) {
                    ...
                    $scope.loadDevices = function() {
                        $scope.callApi({api_id: 1000, command: "registered_devices"}, 
                            function(data, status, headers, config) {
                                ...
                                console.info("RM Bridge Devices:\n" + JSON.stringify(data));

                                if(data.code == 0) {
                                    // Only keep RM devices
                                    rm_devices = [];
                                    for(var i=0; i<data.list.length; i++) {
                                        var type = data.list[i].type;
                                        
                                        if(type == "RM1" || type == "RM2" ||
                                           type == "10001" || type == "10002" || type == "10026") {
                                                rm_devices.push(data.list[i]);
                                        }
                                    }

                                    data.list = rm_devices;     
                    ...
                    $scope.freqScan = function() {
                        $scope.code = undefined;
                        $scope.shortcut_url = undefined;
                        $scope.shortcut_name = $scope.code_name;
                        
                        $scope.callApi({api_id: 1008, command: "freqscan", duration: 7000, mac: $scope.device.mac},
                            function(data, status, headers, config) {
                                if(data.code == 0) {
                                    $scope.working = false;
                                }
                    ...
                    $scope.freqScanCancel = function() {
                        $scope.code = undefined;
                        $scope.shortcut_url = undefined;
                        $scope.shortcut_name = $scope.code_name;
                        
                        $scope.callApi({api_id: 1009, command: "freqscan_learn_cancel", mac: $scope.device.mac},
                            function(data, status, headers, config) {
                                if(data.code == 0) {
                                    $scope.working = false;
                                }
